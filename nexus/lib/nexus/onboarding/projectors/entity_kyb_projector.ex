defmodule Nexus.Onboarding.Projectors.EntityKybProjector do
  ...
  project(%EntityProfileSubmitted{} = event, metadata, fn multi ->
    require OpenTelemetry.Tracer
    Tracing.extract_and_set_context(metadata)

    OpenTelemetry.Tracer.with_span "Projector.Onboarding.EntityProfileSubmitted" do
      attrs = %{
        id: Uniq.UUID.uuid7(),
        org_id: event.org_id,
  ...
  project(%UBOsDeclared{} = event, metadata, fn multi ->
    require OpenTelemetry.Tracer
    Tracing.extract_and_set_context(metadata)

    OpenTelemetry.Tracer.with_span "Projector.Onboarding.UBOsDeclared" do
  ...
  project(%KYBDocumentUploaded{} = event, metadata, fn multi ->
    require OpenTelemetry.Tracer
    Tracing.extract_and_set_context(metadata)

    OpenTelemetry.Tracer.with_span "Projector.Onboarding.KYBDocumentUploaded" do
      attrs = %{
  ...
  project(%KYBReviewCompleted{} = event, metadata, fn multi ->
    require OpenTelemetry.Tracer
    Tracing.extract_and_set_context(metadata)

    OpenTelemetry.Tracer.with_span "Projector.Onboarding.KYBReviewCompleted" do
      query = from(p in EntityProfile, where: p.org_id == ^event.org_id)
