defmodule NexusWeb.InvitationEmail do
  ...
  def send_biometric_invitation(name, email, role, magic_link) do
    new()
    |> to({name, email})
    |> from({"#{brand_name()} Platform", "noreply@nexus.finance"})
    |> subject("Your #{brand_name()} Access — Anchor Your Identity")
  ...
  defp text_body(name, role, magic_link) do
    ...
    Your application to #{brand_name()} has been approved. You have been provisioned as: #{role}.
    ...
    If you did not request access to #{brand_name()}, disregard this email.

    — The #{brand_name()} Platform Team
    """
  end
